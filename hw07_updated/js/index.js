...
const filterByUser = () => {
  ...
  let url = "https://alyssa-homework-7.herokuapp.com/photos"
  if (user_id != "all") {
    url += "?user_id=" + user_id
  }
  fetch(url)
...
const likePhoto = () => {
  const photo_id = getCurrentPhoto().id;
  let url = "https://alyssa-homework-7.herokuapp.com/photos/" + photo_id
  fetch(url, {
       method: 'PATCH',
       headers: {
...
const attachEventHandlers = () => {
    ...
    document.querySelector('#like_button').onclick = likePhoto;
};
